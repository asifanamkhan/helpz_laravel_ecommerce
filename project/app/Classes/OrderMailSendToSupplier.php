...
class OrderMailSendToSupplier
{

    public function sendOrderMail(array $mailData,$id)
    {
        ...
        try{
            Mail::send('admin.email.mailbody',$data, function ($message) use ($objDemo,$id) {
                ...
                $order = ProductAssignToSupplier::where('invoice_no',$id)->get();
                $fileName = public_path('assets/prints/').str_random(4).time().'.pdf';
                $pdf = PDF::loadView('print.product_assign_to_supplier', compact('order'))->save($fileName);
